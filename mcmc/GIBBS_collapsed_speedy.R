...
source("prob_ik_nonempty_function.R")
source("marg_dag.R")
source("normalize_weights.R")

library(Rcpp)
sourceCpp("counts_def.cpp")

library(abind)
library(plyr)
library(prodlim)

Gibbs_collapsed_speedy <- function(Y, S, burn_in, a_pi, b_pi, a_alpha, b_alpha, a, A_constr = NULL) {
  ...
  #  t1 = proc.time()
    ...
    for(k in 1:(K_star - 1)){
      ...
      rows_k = which(xi == k)
      Ck_family  = match_counts_one_cluster_all_rows_fast(Y, rows_k, Dags[,,k], I.cal,
                                                          mode = 1, base = 0)
      Ck_parents = match_counts_one_cluster_all_rows_fast(Y, rows_k, Dags[,,k], I.cal,
                                                          mode = 0, base = 0)
      nk = sum(xi == k)
      member = xi == k
      
      afa = fa_prod_div(a, I.cal, fa.k.list)
      apa = fa_prod_div(a, I.cal, pa.k.list)
      
      logProbs[, k] = log(nk - member) + rowSums(log(afa + Ck_family - matrix(member, nrow = n, ncol = q)) -
                                                log(apa + Ck_parents - matrix(member, nrow = n, ncol = q)))
  ...
  return(list(DAG = A_chain, Xi = Xi_chain, alpha_0 = alpha_0_chain,
              ...
              #t_part1 = t_part1, t_part2 = t_part2, t_part3 = t_part3,
              #t_all = t_all))
