...
pa = function(j, dag){
  return(which(dag[, j]!=0))
}

fa = function(j, dag){
  return(c(j, which(dag[, j]!=0)))
}
...
marg_j = function(j, a, N, I.cal, I.size, dag){
  ...
  N.j.obs   = aggregate(N$freq, by = as.list(N[fa.j]), FUN = sum)$x  # frequncies of the combinations of levels that we observe
